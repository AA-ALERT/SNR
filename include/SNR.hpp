...
std::string *getMaxDMsSamplesOpenCL(const snrConf &conf, const std::string &dataName, const AstroData::Observation &observation, const unsigned int downsampling, const unsigned int padding)
{
    ...
    *code = "__kernel void getMax_DMsSamples_" + std::to_string(nrSamples) + "(__global const " + dataName + " * const restrict time_series, __global " + dataName + " * const restrict max_values, __global unsigned int * const restrict max_indices, __global " + dataName + " * const restrict stdevs) {\n"
        ...
        "reductionMAX[get_local_id(0)] = max_0;\n"
        "reductionSAM[get_local_id(0)] = maxSample_0;\n"
        ...
        "for ( unsigned int value_id = get_local_id(0); threshold > 0; threshold /= 2 ) {\n"
            "if ( (value_id < threshold)) {\n"
                "delta = reductionMEA[sample + threshold] - mean_0;\n"
                "counter_0 += reductionCOU[sample + threshold];\n"
                "mean_0 = ((reductionCOU[sample] * mean_0) + (reductionCOU[sample + threshold] * reductionMEA[sample + threshold])) / counter_0;\n"
                "variance_0 += reductionVAR[sample + threshold] + ((delta * delta) * ((reductionCOU[sample] * reductionCOU[sample + threshold]) / counter_0));\n"
                ...
                "}\n"
                "reductionCOU[sample] = counter_0;\n"
                "reductionMAX[sample] = max_0;\n"
                "reductionSAM[sample] = maxSample_0;\n"
                "reductionMEA[sample] = mean_0;\n"
                "reductionVAR[sample] = variance_0;\n"
            "}\n"
            "barrier(CLK_LOCAL_MEM_FENCE);\n"
        "}\n"
